package jshelper_test

import (
	"reflect"
	"testing"

	"github.com/cameronharro/hs-workflow-tester/internal/jshelper"
)

func TestRunPreOptionFunction(t *testing.T) {
	type TestCase struct {
		Name      string
		Event     jshelper.PreOptionEvent
		Function  string
		ExpectErr bool
		ExpectVal jshelper.PreOptionCallback
	}
	testCases := []TestCase{
		{
			Name: "Returns empty callback ",
			Function: `exports.main = (_, callback) => {
				return callback({})
			}`,
			ExpectErr: true,
		},
		{
			Name: "Returns valid callback",
			Function: `exports.main = (_, callback) => {
				return callback({httpMethod: "POST", body: {foo: "bar"}})
			}`,
			ExpectErr: false,
			ExpectVal: jshelper.PreOptionCallback{HttpMethod: jshelper.Post, Body: map[string]any{"foo": "bar"}},
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			result, err := jshelper.RunPreOptionFunction(testCase.Event, testCase.Function)
			if err != nil != testCase.ExpectErr {
				if err == nil {
					t.Fatal("Expected Error but got nil")
				}
				t.Fatal(err.Error())
			}
			if !reflect.DeepEqual(result, testCase.ExpectVal) {
				t.Fatalf("Expected %s, got %s", testCase.ExpectVal, result)
			}
		})
	}
}
